...
export class MusicFilterStates {
  ...
  setTagValues(tagValues: { id: number; name: string }[]) {
  ...
  addTagValue(tagValue: { id: number; name: string }) {
    if (!this.filterOptions.tags) {
      this.filterOptions.tags = {};
    }
  ...
  removeTagValue(tagValue: { id?: number; name?: string }) {
  ...
  toggleTagValue(tagValue: { id: number; name: string }) {
    if (
  ...
  changeMusicOrdering (orderByColumn: ColumnOrderNameType, orderByDirection: ColumnOrderDirectionType, vyAuthToken?: string) {

    if (this.filterOptions.orderByColumn !== orderByColumn || this.filterOptions.orderByDirection !== orderByDirection) {
        this.setOrderBy({ orderByColumn: orderByColumn, orderByDirection: orderByDirection });
        this.triggerFilteredRequest(vyAuthToken);
    }
    else {
        this.setOrderBy({});
        this.triggerFilteredRequest(vyAuthToken);
