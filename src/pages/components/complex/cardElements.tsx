...
import { VyAudio } from "../vyAudioPlayer";
...
export function MusicTagList({ musicData, hrefRoot }: { musicData: MusicCardData, hrefRoot: string }) {
    return (
        <ul id="mcard-template-tags" className="card-body p-1 my-1 vy-tag-container">
            {musicData.tags.map((tagItem, index) => (
                <a key={`${musicData.id}-${index}`} href={`${hrefRoot}?tagsAny=${tagItem.name}`}>
                    <li className="d-inline-block rounded-pill fs-small mb-1 py-1 px-2 vy-bg-dark-beige vy-tag-text">{tagItem.name}</li>
                </a>
            ))}
        </ul>
    );
